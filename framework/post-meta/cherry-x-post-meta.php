...
if ( ! class_exists( 'Cherry_X_Post_Meta' ) ) {
	...
	class Cherry_X_Post_Meta {
		...
		public function save_meta_mod( $post_id ) {
			...
			// Prevent autosave from triggering meta update
			if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
				return;
			}
			...
			foreach ( sanitize_text_field(wp_unslash($_POST[ $meta_key ])) as $key => $value ) {
		...
		public function save_meta_option( $post_id ) {
			...
			foreach ( $this->args['fields'] as $key => $field ) {
				...
				$value = $this->sanitize_meta( $key, sanitize_text_field(wp_unslash($_POST[ $key ])) );
