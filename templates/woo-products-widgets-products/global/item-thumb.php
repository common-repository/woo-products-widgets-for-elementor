...
<div class="woo-products-product-thumbnail">
	<a href="<?php echo esc_url( get_permalink() ) ?>" rel="bookmark"><?php echo sprintf("%s", wp_kses_post($thumbnail)); ?></a>
	<div class="woo-products-product-img-overlay"></div><?php
		if ( null != $sale_badge && 'yes' === $this->get_attr( 'show_badges' ) ) {
			echo sprintf( '<div class="woo-products-product-badges">%s</div>', wp_kses_post($sale_badge) );
		}
	?></div>